...
<template>
    <div class="h-full relative pt-2" :class="options.length == 0 ? 'opacity-40 pointer-events-none ' : ''">
        <div class="absolute bottom-0 right-0 lg:ml-0 lg:mr-2 flex items-center justify-center gap-2 w-full">
            ...
            <button :class="selectedOption == null ? 'bg-fipu_blue/50 cursor-not-allowed' : 'bg-fipu_dark_blue hover:bg-fipu_blue dark:hover:bg-fipu_blue'"
                @click="selectedOption != null ? chatStore.addConversation(selectedOption) : ''" class="flex items-center justify-center shadow-sm min-w-[40px] h-[40px] w-[40px] drop-shadow
