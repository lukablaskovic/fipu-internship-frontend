		...
		<div class="flex items-center pt-2.5 pb-2.5 lg:pt-4 lg:pb-4 relative hover:bg-white dark:hover:bg-gray-800/25" :class="selected ? 'border-l-2 bg-gradient-to-r pl-1.5 pr-2 lg:pr-4 lg:pl-3.5 from-sky-200 dark:from-sky-950/50 to-transparent border-fipu_dark_blue dark:border-fipu_dark_blue' : 'pl-2 pr-2 lg:pr-4 lg:pl-4 hover:rounded'">
			...
			<div v-if="mainStore.currentUser.account_type == 'admin'" class="absolute right-1 lg:right-3 bottom-0 lg:bottom-2 flex gap-0 lg:gap-2">
				<div
					...
					@click="
						cstatus == 'stared' ? (cstatus = 'normal') : (cstatus = 'starred');
						chatStore.updateConversationStatus('starred', conversation);
					">
					<BaseIcon class="hover:text-fipu_dark_blue hover:cursor-pointer" :size="layoutStore.isLg ? 22 : 18" :path="(conversation.status == 'starred' && cstatus == 'starred') || cstatus == 'starred' ? mdiStar : mdiStarOutline"></BaseIcon>
				</div>
				<div
					@mouseover="hover = true"
					@mouseleave="hover = false"
					@click="
						cstatus == 'archived' ? (cstatus = 'normal') : (cstatus = 'archived');
						chatStore.updateConversationStatus('archived', conversation);
					">
					<BaseIcon class="hover:text-fipu_dark_blue hover:cursor-pointer" :size="layoutStore.isLg ? 22 : 18" :path="(conversation.status == 'archived' && cstatus == 'archived') || cstatus == 'archived' ? mdiArchive : mdiArchiveOutline"></BaseIcon>
				</div>
			</div>
