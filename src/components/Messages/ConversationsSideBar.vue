...
<template>
    <div class="flex flex-row flex-shrink-0 bg-gray-100 dark:bg-gray-900 p-4 absolute lg:relative h-full min-w-[256px] w-96 transition-all duration-500 lg:duration-0 z-20"
        :class="[chatStore.collapsed ? '-left-96 lg:-left-0' : '-left-0']">
        <div class="absolute top-0 -right-8 lg:hidden">
            <div class="w-8 h-8 flex justify-center items-center rounded-full" @click="chatStore.collapsed = !chatStore.collapsed">
                <BaseIcon class="hover:text-fipu_dark_blue hover:cursor-pointer" w="40" h="40" :size="36" 
                :path="chatStore.collapsed ? mdiMenuRightOutline : mdiMenuLeftOutline"></BaseIcon>
            </div>
        </div>
        <div class="flex flex-col w-full h-full pl-1 lg:pl-4 pr-1 lg:pr-4 py-4 -mr-4">
            ...
            <div v-if="chatStore.update" class="flxe flex-col">
                ...
                :selected="chatStore.selectedConversationID == c.id"/>
            </div>
