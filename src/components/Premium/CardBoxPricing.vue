...
const isCollapsedComputed = computed(() => isCollapsible.value && isCollapsed.value);
</script>

<template>
	<CardBox :class="[isMain ? 'shadow-2xl' : 'lg:my-12']" rounded="rounded-2xl" has-component-layout>
		<header class="p-6 border-gray-50 dark:border-slate-800 border-b" @click.prevent="isCollapsed = !isCollapsed">
			<BaseLevel mobile>
				<div>
					...
					<PillTag :label="item.label" :color="item.labelType" class="mt-3" small />
				</div>
				...
				<BaseLevel mobile>
					...
					<BaseIcon v-if="isCollapsible" :path="isCollapsedComputed ? mdiChevronUp : mdiChevronDown" size="48" w="w-16" h="h-16" class="ml-6 lg:hidden" />
				</BaseLevel>
			</BaseLevel>
		</header>
		...
		<CardBoxComponentFooter :class="{ 'hidden lg:block': isCollapsedComputed }">
			<BaseButtons>
				<BaseButton class="flex-1" color="info" label="Subscribe" :outline="!isMain" />
			</BaseButtons>
