...
<template>
	<div v-if="tip" class="absolute z-20 flex flex-col pt-1 animate-fade-in" :class="[top ? 'bottom-full flex-col-reverse' : 'top-full', { 'inset-x-auto': !left && !right, 'left-0': left, 'right-0': right }]">
		<div
			class="text-black text-opacity-80 h-3 dark:text-gray-800"
			:class="{
				'mx-auto': !left && !right,
				'ml-auto mr-1.5': right,
				'mr-auto ml-1.5': left,
			}">
