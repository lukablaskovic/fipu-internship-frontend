...
const User = {
  ...
  async updatePassword(updates, retries = 3) {
    while (retries > 0) {
      try {
        ...
        if (retries === 1) {
          console.error(
            `Failed to update password after multiple attempts:`,
            error
          );
          return null;
        }
  ...
  async getAllAdmins(retries = 3) {
    while (retries > 0) {
      try {
        ...
        if (retries === 1) {
          console.error(
            "Failed to fetch admins after multiple attempts:",
            error
          );
          return null;
        }
  ...
  async getAllUsers(retries = 3) {
    while (retries > 0) {
      try {
        ...
        if (retries === 1) {
          console.error(
            "Failed to fetch users after multiple attempts:",
            error
          );
          return null;
        }
    ...
    while (retries > 0) {
      try {
        let result = await AxiosWrapper.get(
          `/users/get_messages/${receiver_id}`
        );
        return result;
      } catch (error) {
        if (retries === 1) {
          console.error(
            "Failed to fetch messages after multiple attempts:",
            error
          );
          return null;
        }
  ...
  async getLastMessage(receiver_id, retries = 3) {
    while (retries > 0) {
      try {
        let result = await AxiosWrapper.get(
          `/users/get_last_message/${receiver_id}`
        );
        return result;
      } catch (error) {
        if (retries === 1) {
  ...
  async addConversation(conversation, retries = 3) {
    while (retries > 0) {
      try {
        ...
        if (retries === 1) {
          console.error(
            "Failed to add conversation after multiple attempts:",
            error
          );
          return null;
        }
  ...
  async getConversations(userId, retries = 3) {
    while (retries > 0) {
      try {
        let result = await AxiosWrapper.get(
          `/users/get_conversations/${userId}`
        );
        return result;
      } catch (error) {
        if (retries === 1) {
  ...
  async updateConversation(conversationId, updates, retries = 3) {
    while (retries > 0) {
      try {
        let result = await AxiosWrapper.patch(
          `/users/update_conversation/${conversationId}`,
          updates
        );
        return result.data;
      } catch (error) {
        if (retries === 1) {
          console.error(
            `Failed to update conversation ${conversationId} after multiple attempts:`,
            error
          );
          return null;
        }
