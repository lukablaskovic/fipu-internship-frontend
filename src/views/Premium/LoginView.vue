		...
		<SectionFormScreen v-slot="{ cardClass }" bg="purplePink" has-promo>
			<CardBox :class="[cardClass, cardClassAddon]" is-form @submit.prevent="submit">
				<FormField label="Login" :error="hasError" help="Please enter your login">
					<FormControl v-model="form.login" :error="hasError" :icon-right="mdiAccount" name="login" placeholder="user@example.com" autocomplete="username" />
				</FormField>

				<FormField label="Password" :error="hasError" help="Click icon to show/hide">
					<FormControl v-model="form.pass" :error="hasError" :tip-right="passShowHideClicked ? null : 'Click to show/hide'" type="password" name="password" placeholder="Password" autocomplete="current-password" @right-icon-click="passShowHideClicked = true" />
				</FormField>
				...
				<template #footer>
					<BaseLevel mobile>
						<BaseButtons>
							<BaseButton label="Login" type="submit" color="info" :tip="hasTip ? 'Click to see error state' : null" />
