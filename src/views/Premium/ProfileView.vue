...
const firstDayOfNextMonth = new Date(date.getFullYear(), date.getMonth() + 1, 1).toLocaleDateString("en-US", dateOptions);

const firstDayOfThisMonth = new Date(date.getFullYear(), date.getMonth(), 1).toLocaleDateString("en-US", dateOptions);
...
const billingHistory = [
	...
	},
	{
		id: 2,
		amount: 24.99,
		date: new Date(date.getFullYear(), date.getMonth() - 1, 1).toLocaleDateString("en-US", dateOptions),
	},
	{
		id: 3,
		amount: 24.99,
		date: new Date(date.getFullYear(), date.getMonth() - 2, 1).toLocaleDateString("en-US", dateOptions),
	},
];
...
<template>
	<LayoutAuthenticated>
		<SectionMain>
			...
			<CardBoxBillingItem v-for="billingHistoryItem in billingHistory" :key="billingHistoryItem.id" :amount="billingHistoryItem.amount" :date="billingHistoryItem.date" />
			...
			<CardBoxPaymentMethod v-for="paymentMethod in paymentMethods" :key="paymentMethod.id" :card-type="paymentMethod.cardType" :card-number="paymentMethod.cardNumber" :card-expires="paymentMethod.cardExpires" :card-owner="paymentMethod.cardOwner" :is-primary="paymentMethod.isPrimary" />
        ...
        <BaseButton label="Show more" color="info" class="flex-1" outline />
			...
			<CardBox class="mb-6">
				...
				<div class="flex items-center justify-between">
					...
					<FormCheckRadio v-model="apiStatusEnabled" type="switch" name="api-status" :input-value="true" />
				</div>
				<BaseDivider />
				<div class="flex items-center justify-between">
					<div class="flex items-center justify-start">
						<p class="mr-3 leading-none">2-factor authentication</p>
						<PillTag :color="twoFactorEnabled ? 'info' : 'danger'" :label="twoFactorEnabled ? 'enabled' : 'disabled'" small />
					</div>

					<FormCheckRadio v-model="twoFactorEnabled" type="switch" name="two-factor" :input-value="true" />
				</div>
				<BaseDivider />
				<div class="flex items-center justify-between">
					<p>
			...
			<div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
				<CardBox is-form @submit.prevent="submitProfile">
					...
					<FormField label="E-mail" help="Required. Your e-mail">
						<FormControl v-model="profileForm.email" :icon="mdiMail" type="email" name="email" required autocomplete="email" />
					</FormField>
