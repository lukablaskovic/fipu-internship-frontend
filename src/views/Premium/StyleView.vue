			...
			<CardBox class="xl:w-8/12 md:mx-auto">
				<div>
					<BaseButtons type="justify-center">
						<BaseButton v-for="(styleBg, style) in styles" :key="style" :color="style === 'white' ? style : `${styleBg} text-white`" :label="style" class="capitalize" rounded-full @click="setStyle(style)" />
					</BaseButtons>
				</div>
			</CardBox>
		</SectionMain>
		...
		<SectionMain>
			<CardBox class="md:w-7/12 lg:w-5/12 xl:w-4/12 md:mx-auto">
				<div class="text-center py-24 lg:py-12 text-gray-500 dark:text-gray-400">
					<BaseButton label="Toggle light/dark" :icon="mdiThemeLightDark" color="contrast" @click="darkModeToggle" />
				</div>
			</CardBox>
		</SectionMain>
	</LayoutAuthenticated>
