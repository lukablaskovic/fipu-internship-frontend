...
<template>
    <SectionSplitLogin bg="blue" class="flex items-start min-h-screen">
        <Transition appear enter-active-class="animate__animated animate__fadeIn" 
            ...
            <div v-if="!transitioning" class="flex flex-col justify-center items-center overflow-hidden h-screen || md:py-16 md:px-4 || transition-all duration-300">
                <div class="flex flex-col md:flex-row flex-shrink h-full || bg-white rounded-lg overflow-hidden || transition-all duration-300
                            px-0 lg:px-6 xl:px-16 2xl:pr-28 2xl:pl-20 || xl:gap-4 2xl:gap-8">
                    <CardBox class="hidden xl:block flex-1 bg-opacity-0 grow p-16 xl:px-0 xl:py-16 2xl:py-24 || transition-all duration-300" centered-content >
                        <img src="login_art.jpg" alt="Login graphics" class="2xl:pr-8 w-full"/>
                    </CardBox>

                    <CardBox class="flex flex-col flex-shrink flex-1 bg-opacity-0 p-8 sm:p-16 xl:pl-2 xl:px-0 xl:py-24 || transition-all duration-300" vertical-centered is-form @submit.prevent="onSubmit">
                        <a href="https://fipu.unipu.hr/" target="_blank">
                            <img src="fipu_unipu.png" alt="fipu logo" class="h-16 mb-6 object-contain"/>
                        </a>
                        ...
                        <h2 class="md:text-sm lg:text-sm 2xl:text-base mb-2 2xl:mb-4 text-justify">
                            ...
                            <a class="hover-underline-animation cursor-pointer text-fipu_text_blue hover:text-fipu_blue" @click="router.push('/moja-praksa')">
                                ovdje
                            </a>
                        </h2>

                        <FormField label="E-mail">
                            <FormControl v-model="loginForm.email" :icon="mdiAccount" name="email" 
                                :error="getFirstErrorForField('email')" autocomplete="username">
                            </FormControl>
                        </FormField>

                        <FormField label="Lozinka">
                            <FormControl v-model="loginForm.password" :icon="mdiAsterisk" name="password" type="password" placeholder="Password" 
                                :error="getFirstErrorForField('password')" @right-icon-click="passShowHideClicked = true" autocomplete="current-password">
                            </FormControl>
                        </FormField>
                        ...
                        <div class="my-2 2xl:my-4 flex items-center before:mt-0.5 before:flex-1 before:border-t before:border-neutral-300 
                        ...
                        <NotificationBar ref="notificationBar" class="animate__animated animate__fadeInUp mt-2" :outline="notificationsOutline">
                            ...
                            <template #right>
                                <BaseButton :icon="mdiClose" :color="notificationsOutline ? 'success' : 'white'"
                                    :outline="notificationsOutline" rounded-full small />
                            </template>
                        </NotificationBar>
                    </CardBox>
                </div>
